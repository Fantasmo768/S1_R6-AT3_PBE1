...
const pedidoModel = {
    ...
    adicionarPedido: async (data_pedido, entrega_urgente, distancia, peso, valor_km, valor_kg, id_cliente) => {
        ...
        try {
            const sqlPedido = 'INSERT INTO pedidos (data_pedido, entrega_urgente, distancia, peso, valor_km, valor_kg, id_cliente) VALUES (?, ?, ?, ?, ?, ?, ?);';
            const values = [data_pedido, entrega_urgente, distancia, peso, valor_km, valor_kg, id_cliente];
            const [rowsPedido] = connection.query(sqlPedido, values);
